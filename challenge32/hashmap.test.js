...
class Hashmap {
  ...
  get(key) {
    ...
    if (this.storage[hash]) {
      let currNode = this.storage[hash].head;
      while (currNode) {
...
describe("challenge32", () => {
    ...
    it(" Should successfuly return the common nodes between two trees", () => {
    ...
    it(" Should check the next of uncommon elements is null in the hash table", () => {
        let hashTable = new Hashmap(5000);
        let expectedResults = [100, 160, 125, 175, 350, 500, 200];
        expect(hashTable.treeIntersection(firstTree, secondTree)).toEqual(
            expectedResults
